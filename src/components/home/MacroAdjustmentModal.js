...
import Animated, { FadeInDown, Easing } from 'react-native-reanimated'; // Agregamos Easing
...
export const MacroAdjustmentModal = ({ visible, onClose, currentStats, onSave }) => {
    ...
    useEffect(() => {
        if (visible && currentStats) {
            setValues({
                calories: currentStats.calories.target.toString(),
                protein: currentStats.protein.target.toString(),
                carbs: currentStats.carbs.target.toString(),
                fats: currentStats.fats.target.toString()
            });
        }
            ...
            <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
                <View className="flex-1 bg-black/60 justify-end">
                    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
                        ...
                        {/* --- CAMBIO AQUÍ: Eliminamos .springify() y usamos Easing --- */}
                        <Animated.View 
